...
ROBOFLOW_WORKSPACE = "surf-pose-estimation"
ROBOFLOW_PROJECT   = "surf-pose-estimation-ysbb5"
ROBOFLOW_VERSION   = "1"
...
# ─────────────────────────────────────────────
def download_weights(api_key, workspace, project, version, dest_path):
    ...
    # Try 1: dataset/version export endpoint
    url1 = f"https://api.roboflow.com/{workspace}/{project}/{version}/yolov8pytorch?api_key={api_key}"
    print(f"    Trying export endpoint...")
    r1 = requests.get(url1, timeout=30)
    if r1.status_code == 200:
        data1 = r1.json()
        weights_url = (
            data1.get("export", {}).get("link")
            or data1.get("link")
            or data1.get("weights")
        )

    # Try 2: model endpoint
    if not weights_url:
        url2 = f"https://api.roboflow.com/{workspace}/{project}/{version}?api_key={api_key}&format=yolov8pytorch"
        print(f"    Trying model endpoint...")
        r2 = requests.get(url2, timeout=30)
        if r2.status_code == 200:
            data2 = r2.json()
            weights_url = (
                data2.get("model", {}).get("weights")
                or data2.get("model", {}).get("weights_url")
                or data2.get("weights")
            )

    if not weights_url:
        print("\n⚠️  Roboflow auto-download failed.")
        print("    Please download the weights manually:")
        print("    1. Go to: https://universe.roboflow.com/surf-pose-estimation/surf-pose-estimation-ysbb5")
        print("    2. Click 'Model' → 'Download' → select 'YOLOv8 PyTorch'")
        print(f"   3. Save the file as:  {dest_path}  (in this folder)")
...
def main():
    ...
    if not Path(WEIGHTS_FILE).exists():
        if ROBOFLOW_API_KEY == "YOUR_API_KEY_HERE":
            ...
            sys.exit(1)
        download_weights(ROBOFLOW_API_KEY, ROBOFLOW_WORKSPACE, ROBOFLOW_PROJECT, ROBOFLOW_VERSION, WEIGHTS_FILE)
    else:
